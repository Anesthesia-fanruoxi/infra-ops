...
func planTestHosts(t *testing.T, ips []string, ha bool, masters map[string]string) []model.StackRunHost {
	...
	for i := 1; i < len(ips); i++ {
		p, _ := json.Marshal(map[string]string{"ha": boolStr(ha), "components": "hdfs,zookeeper,yarn,spark,flink,hbase,hive,metastore_db,trino"})
...
// 运行 stackBigdataRole，其落点与全量 Plan 逐字节一致。
