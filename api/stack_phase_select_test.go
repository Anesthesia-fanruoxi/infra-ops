...
func TestBigdataNodeScriptAssets(t *testing.T) {
	...
	script, err := bp.LoadPhase("cluster", "node")
	if err != nil {
...
func TestValidateBigdataAdd_DepSatisfiedByInstalled(t *testing.T) {
	installed := []string{"hdfs", "zookeeper", "yarn", "spark", "flink", "hbase", "metastore_db", "hive"}
	if err := validateBigdataAdd([]string{"trino"}, installed); err != nil {
		t.Fatalf("已安装 hive 时加装 trino 应通过，got: %v", err)
	}
	// 反向：集群无 hive 时加装 trino 仍须拒绝
	if err := validateBigdataAdd([]string{"trino"}, []string{"hdfs", "zookeeper"}); err == nil {
		t.Fatal("缺少 hive 时加装 trino 应被拒绝")
	}
	// 同时加装 hive + trino 也应通过
	if err := validateBigdataAdd([]string{"hive", "trino"}, []string{"hdfs", "zookeeper"}); err != nil {
		t.Fatalf("同时加装 hive+trino 应通过，got: %v", err)
	}
	// HBase 依赖 ZooKeeper 同理
	noHbase := []string{"hdfs", "zookeeper", "yarn", "hive"}
	if err := validateBigdataAdd([]string{"hbase"}, noHbase); err != nil {
		t.Fatalf("已安装 zookeeper 时加装 hbase 应通过，got: %v", err)
	}
	if err := validateBigdataAdd([]string{"hbase"}, []string{"hdfs", "hive"}); err == nil {
		t.Fatal("缺少 zookeeper 时加装 hbase 应被拒绝")
	}
	// 基础约束不回退：重复加装 / 加装 hdfs / 非法组件
	if err := validateBigdataAdd([]string{"hive"}, installed); err == nil {
		t.Fatal("重复加装已安装组件应被拒绝")
	}
	if err := validateBigdataAdd([]string{"hdfs"}, installed); err == nil {
		t.Fatal("加装 hdfs 应被拒绝")
	}
	if err := validateBigdataAdd([]string{"nope"}, installed); err == nil {
		t.Fatal("非法组件应被拒绝")
	}
}
