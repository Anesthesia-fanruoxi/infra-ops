...
func (h *orchHandler) executeRun(orchID, runID int64) {
	...
	for _, def := range stepsDef {
		...
		for _, hid := range deploy.DedupInt64(hostIDs) {
	...
	execCell := func(cell repo.RunStepRef, def *model.OrchestrationStep) {
		...
		for a := 1; a <= attempts; a++ {
			...
			rendered, rerr := deploy.RenderScript(tpl.Script, tpl.Variables, stepParams)
			if rerr != nil {
				lastErr = rerr.Error()
				break // 渲染错误重试无意义
			}
			...
			if hint := deploy.CheckRequires(h.hostRepo, h.credRepo, h.cryptoS, h.sshC, cell.HostID, deploy.TemplateRequires(tpl)); hint != "" {
			...
			out, execErr := deploy.ExecHostWith(h.hostRepo, h.credRepo, h.cryptoS, h.sshC, cell.HostID, rendered, onLog)
			lastOut = out
			if execErr != nil {
		...
		if status == "success" {
			...
			if err := shared.RegisterTemplateServices(h.tplRepo, cell.HostID, cell.HostIP, def.TemplateID, stepParams); err != nil {
