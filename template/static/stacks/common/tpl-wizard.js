...
;(function () {
  ...
  P.tpl.wizard = `  <el-dialog v-model="wizardVisible" :title="wizardTitle" width="85%" top="6vh" class="stk-wizard-dialog" :close-on-click-modal="false" @closed="resetWizard">
    ...
    <div v-show="step===1">
      ...
      <div v-else>
      <div v-if="stackTabs.length > 1" class="stk-cat-tabs">
        <button v-for="t in stackTabs" :key="t.key" type="button" class="stk-cat-tab" :class="{'stk-cat-tab--active': catFilter===t.key}" @click="catFilter=t.key">
          {{t.label}}<span class="stk-cat-tab-num">{{t.count}}</span>
        </button>
      </div>
      <div v-if="activeCatTab.hint" class="stk-cat-summary">
        <span class="stk-cat-summary-label">{{activeCatTab.label}}</span>
        <span class="stk-cat-summary-text">{{activeCatTab.hint}}</span>
      </div>
      <div class="stk-card-grid">
        <button v-for="s in visibleStacks" :key="s.key" type="button" class="stk-card" :class="{'stk-card--sel': selectedKey===s.key}" @click="selectStack(s)">
          ...
          <div v-if="catFilter==='all' && stackTabs.length > 1" class="stk-card-meta"><span class="stk-cat-chip">{{stackCatLabel(s)}}</span></div>
        </button>
      </div>
      <div v-if="!visibleStacks.length && !stacksLoading" class="deploy-placeholder">该分类下暂无可用套件</div>
    ...
    <div v-show="step===2">
      <!-- 镜像源（平台级能力）只在第 3 步表达：由套件声明的 Type=registry 变量渲染成「自建仓库」下拉。
           第 2 步不再有「直连拉取 / hub 镜像主机」栏——同一件事两处表达只会互相打架。 -->
      ...
      <!-- Redis 等仍用主从/引导节点单选 -->
      <template v-else>
        <!-- step2Split「倒品字」两栏：上左主机勾选、上右套件表单，下方通栏角色计划预览；
             未开启时保持原纵向顺序（套件表单在最上方，redis 等沿用） -->
        <div class="stk-step2-grid" :class="{'stk-step2-grid--split': step2Split}">
          <div v-if="step===2 && step2FormComp" class="stk-step2-slot-form">
            <component :is="step2FormComp" :ctx="selfCtx" />
          </div>
          <div class="stk-step2-slot-host">
    ...
    <div v-show="step===3" class="stk-param-step">
      ...
      <div class="deploy-host-var-fields stk-shared-fields">
        <div class="deploy-host-var-field" v-for="v in visibleSharedVars" :key="v.name">
          ...
          <!-- 自建仓库选择器（Type=registry）：候选与部署中心「hub 镜像主机」同一台账与口径，0=不使用 -->
          <el-select v-else-if="v.type==='registry'" :model-value="registryPick" @change="onRegistryPick" size="small" style="width:100%" :loading="registriesLoading" placeholder="选择自建仓库（不用则直连官方源）" :teleported="false">
            <el-option label="不使用（直连官方源）" :value="0" />
          ...
          <el-checkbox v-if="v.type==='registry' && registryPick" :model-value="hubAutoInsecure" size="small" @change="e => hubAutoInsecure = e">自动配置目标机 insecure-registries（会重启其 Docker）</el-checkbox>
          <span v-if="v.type==='registry' && registriesLoaded && !onlineRegistries.length" class="deploy-var-hint">没有可用的自建仓库：请先对目标机执行「安装 Docker Registry」模板部署镜像仓库，并保持主机在线</span>
          <span v-else-if="v.type==='registry'" class="deploy-var-hint">{{registryPick ? '镜像先在该仓库预热，目标机改从它拉取' : '各目标机直接从源仓库拉取镜像'}}</span>
        </div>
      </div>
      <!-- 第 3 步套件插槽：套件自定义 UI（如 rabbitmq 延迟插件资产条）。须与触发它的共享参数同屏 -->
