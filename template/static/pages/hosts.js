...
<div class="hosts-page">
  ...
  <el-drawer v-model="drawer" :title="detail?.name" size="560px">
    ...
    <div class="detail-section" v-if="baseInstalls.length"><h4>基础建设 <span style="color:var(--text-faint);font-size:11px;font-weight:400">（{{baseInstalls.length}} 项）</span></h4><div class="hc-badges">
      <span v-for="it in baseInstalls" :key="it.id" class="hc-base-badge" :title="it.template_name + ' · ' + it.updated_at">{{it.template_name}}</span>
    </div></div>
    <div class="detail-section" v-if="clusters.length || clustersLoading"><h4>套件集群 <span v-if="clusters.length" style="color:var(--text-faint);font-size:11px;font-weight:400">（{{clusters.length}} 个）</span></h4>
      <div v-if="clustersLoading" style="color:#94A3B8;font-size:12px">加载中…</div>
      <div v-else class="hc-cluster-list">
        <div v-for="cl in clusters" :key="cl.instance_id" class="hc-cluster-card">
          <div class="hc-cluster-header">
            <span class="hc-cluster-name">{{cl.stack_name}}</span>
            <span class="hc-cluster-mode">{{modeLabel(cl.mode)}}</span>
            <span class="hc-cluster-status" :class="'st-' + cl.status">{{clusterStatusText(cl.status)}}</span>
            <el-button size="small" text type="primary" :loading="cl._verifying" @click="verifyCluster(cl)">探活</el-button>
          </div>
          <div class="hc-cluster-meta">
            <span class="hc-cluster-inst">{{cl.instance_name}}</span>
            <span class="hc-cluster-role" :class="cl.role === 'master' ? 'is-master' : 'is-replica'">{{cl.role === 'master' ? '主' : '从'}}</span>
          </div>
          <div class="hc-peer-list" v-if="cl.peers && cl.peers.length">
            <div v-for="p in cl.peers" :key="p.host_ip" class="hc-peer-item">
              <span class="hc-peer-dot" :class="p.role === 'master' ? 'is-master' : 'is-replica'"></span>
              <span class="mono hc-peer-ip">{{p.host_ip}}</span>
              <span class="hc-peer-role">{{p.role === 'master' ? '主' : '从'}}</span>
            </div>
          </div>
          <div class="hc-svc-list" v-if="cl.services && cl.services.length">
            <div v-for="svc in cl.services" :key="svc.id" class="hc-svc-item">
              <span class="hc-svc-name">{{svc.service_name}}</span>
              <span class="mono hc-svc-url">{{svc.url || '（无地址）'}}</span>
              <span class="hc-svc-copy" @click="copyText(svc.url)" v-if="svc.url"><svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg></span>
    ...
    <div class="detail-section" v-if="svcInstalls.length"><h4>其他服务 <span style="color:var(--text-faint);font-size:11px;font-weight:400">（{{svcInstalls.length}} 项）</span></h4>
      <div class="host-install-list">
        <div v-for="svc in svcInstalls" :key="svc.id" class="host-service-item">
    ...
    <div class="detail-section"><h4>部署记录 <span v-if="installs.length" style="color:var(--text-faint);font-size:11px;font-weight:400">（{{installs.length}} 项）</span></h4>
      <div v-if="installsLoading" style="color:#94A3B8;font-size:12px">加载中…</div>
      <div v-else-if="!installs.length" style="color:#94A3B8;font-size:12px">该主机尚未执行过安装</div>
      <div v-else class="host-install-list">
        <div v-for="it in installs" :key="it.id" class="host-install-item">
          <span class="tag-badge" :class="installTagClass(it.template_name)">{{it.template_name}}</span>
  ...
  data() {
    return { list: [], loading: false, pg: 1, total: 0, filter: { tag: '', status: '', name: '', ip: '' }, sort: JSON.parse(localStorage.getItem('hosts-sort') || '{"field":"name","order":"asc"}'), viewMode: localStorage.getItem('hosts-view-mode') || 'table', drawer: false, detail: null, installs: [], installsLoading: false, services: [], clusters: [], clustersLoading: false, dialog: false, form: {}, creds: [], credLoading: false, credsLoaded: false, saving: false, eventSource: null, batchDialog: false, batchForm: { credential_id: null, ips: '', port: 22, tag: 'other', remark: '', auto_test: true }, batchSubmitting: false, batchTimer: null, preview: { count: 0, dup: 0, err: '' }, result: { show: false, created: 0, failed: 0, skipped: 0, results: [] } }
  },
  computed: {
    baseInstalls() { return this.installs.filter(it => !/^套件[:]/.test(it.template_name)) },
    svcInstalls() {
      const clusterSvcNames = new Set()
      this.clusters.forEach(cl => { (cl.services || []).forEach(s => clusterSvcNames.add(s.service_name)) })
      return this.services.filter(s => !clusterSvcNames.has(s.service_name))
    },
  },
  ...
  methods: {
    ...
    openDetail(row) {
      ...
      this.services = []
      this.clusters = []; this.clustersLoading = true
      api.get('/hosts/' + row.id + '/clusters').then(r => { this.clusters = r.data || []; this.collectServices() }).catch(() => {}).finally(() => { this.clustersLoading = false })
    },
    collectServices() {
      const out = []
      const seen = new Set()
      this.clusters.forEach(cl => {
        (cl.services || []).forEach(s => {
          if (!seen.has(s.service_name)) { seen.add(s.service_name); out.push(s) }
        })
      })
      this.services = out
    },
    openService(url) { if (url) window.open(url, '_blank', 'noopener') },
    copyText(text) { if (!text) return; navigator.clipboard.writeText(text).then(() => this.$message.success('已复制')).catch(() => this.$message.error('复制失败')) },
    modeLabel(mode) { const m = { replication: '主从', sentinel: '哨兵', cluster: '集群', kraft: 'KRaft', zk: 'ZooKeeper', standalone: '单机', single: '单节点', ha: '高可用' }; return m[mode] || mode },
    clusterStatusText(s) { return { ready: '就绪', deploying: '部署中', partial: '部分成功', failed: '失败', uninstalled: '已卸载' }[s] || s },
    installTagClass(name) {
      if (/套件[:]/.test(name)) return 'redis'
      if (/docker|nginx|mysql|openresty|k8s|container/i.test(name)) return 'k8s'
      return 'other'
    },
    async verifyCluster(cl) {
      cl._verifying = true
      try {
        const r = await api.post('/stacks/instances/' + cl.instance_id + '/verify', {}, { timeout: 120000 })
        if (r.code === 0 && r.data) {
          const d = r.data
          this.$notify({ title: d.ok ? '探活通过' : '探活异常', type: d.ok ? 'success' : 'warning',
            message: d.summary + (d.hint ? '\n' + d.hint : ''), duration: 5000 })
        } else { this.$message.error(r.message || '探活失败') }
      } catch (e) { this.$message.error('探活请求超时或失败') } finally { cl._verifying = false }
    },
