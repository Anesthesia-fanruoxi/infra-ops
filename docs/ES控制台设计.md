...
与「生命周期 · 可视化创建 · 冷热温」**同构**（左表单 + 竖分隔 + 右实时 JSON 预览，共用 `.es-qc-*` 版式类）：把手工 `PUT` 出来的模板里真正变化的那几个值收进表单，其余按既有值给默认。表单五段：

| 表单段 | 字段 | 默认值来源 |
| --- | --- | --- |
| 基础 | 模板名称 / `index_patterns`（可多条）/ `priority` | priority 21（通配兜底用 20） |
| 生命周期 | 开关 + 策略下拉（开时必选） | `GET /_ilm/policy?system=0`（系统策略后端已剔除）；**候选唯一时自动选中**，多个不猜 |
| 索引结构 | 主分片 / 副本 / `refresh_interval` / `translog.durability` / `translog.flush_threshold_size` | 3 / 0 / 10s / async / 512m |
| 初始分层 | `_tier_preference` | `data_hot`；候选按 `GET /_nodes` 的实际角色过滤，无对应节点的层禁用 |
| 时间字段 | 字段名 + `format` | 由 `POST /index-pattern/probe` 从命中索引的 mapping 探测候选与 format |

**公共层（组件模板）**：把横切配置抽成一个组件模板供多个索引模板 `composed_of` 引用——「改一处、处处生效」。切分口径：

> **公共层管「数据怎么流转、怎么写」（`lifecycle` / `_tier_preference` / `refresh_interval` / `translog`）；
> 索引模板管「数据怎么切分、怎么建模」（`number_of_shards` / `number_of_replicas` / `mappings`）。**

分片数**不得**进公共层：它是最典型的逐模板差异项，进公共层会把所有共享者锁成同一值。公共层为「新建」时，保存会**依次发两条请求**（组件模板必须先于引用它的索引模板），二次确认把两条一起完整回显；覆盖已被引用的公共层时，界面点名「已被 N 个索引模板引用」及具体名字。

两处会静默错配的地方写进实现：

- **`_simulate` 遇上尚未创建的组件模板**：`composed_of` 在 ES 侧解析不到会报 not found。所以「新建公共层」这一路先把公共项**内联展开**再模拟，模拟的是**保存后的合并结果**，口径在抽屉里写明；「复用已有组件模板」这一路保留 `composed_of`，交给 ES 真解析。
- **策略迁往的层集群没有**：ES 不报错，索引会静默停在原层等待。故用策略 `phases.*.actions.allocate.include._tier_preference` 与节点实际角色比对，缺层时在表单上直接告警。

**两个入口并存，不是二选一**：可视化覆盖横切配置与时间字段这条主线；`aliases` / `analyzer` / `dynamic_templates` 等仍走「JSON 直接提交」。做窄的部分明确不做，胜过把表单做成半成品。

**pattern 重叠检测**：新建时用「通配符之前的字面前缀互为前缀」这一保守近似，对每个已有的重叠模板给出「谁生效」（比 `priority`）。多份模板共享同一前缀时（如 `xafq_*` 兜底 + 若干 `xafq_xxx*` 业务表），这是最容易踩的坑。

#### 对话框版式契约（两轮都是量出来的，不是估算）

表单内容是对话框里最长的一列，版式必须按「内容可以无限长、视口不会变长」来设计，否则底部按钮会被顶出屏幕、右侧预览会留出死白。落定的口径（`.es-tqc-dialog`，全部规则带该前缀，共用 `.es-qc-*` 版式类的生命周期对话框不受影响）：

> **整框 = 高度受限的 flex 列 → body 只当容器（不滚）→ 左右两列吃满剩余高度并各自滚 → 预览框吃掉右列剩余高度。**

三处反直觉的地方，逐条都上锁了：

- **body 不能自己滚**。初版让 body 滚，结果右列被撑成「左侧表单有多长它就有多长」（实测 1232px），预览框再怎么设高度也只是根长条，可见区下方照样空着。滚动必须下放到两列。
- **预览框必须显式给 `min-height`**。只写 `flex-grow` 是不够的 —— `<pre>` 的 flex 自动最小尺寸等于整段 JSON 的高度，不显式覆盖它根本缩不下来。
- **整框 `max-height` 必须用 `calc(100vh - 15vh - 60px)`**。Element Plus 的 `--el-dialog-margin-top` 默认就是 `15vh`，只写 `max-height: 85vh` 会溢出。

实测（900px 视口）：右列无死白、竖分隔线随两列铺满、左表单列可滚（1232 / 478）、预览框 314px；754px 视口同样成立（预览框压到 190px，短视口下的必然取舍 —— 旧实现固定 378px 好看，代价是下方空出 690px）。两条版式契约（视口自适应 + 右列铺满）在 `script/_fe_smoke.js` 第 13 项 13j 里逐条断言，`script/measure_es_tpl_quick.html` 渲染台另带 `?stack=true` / `?fixedprev=true` 两个注入开关做反向验证。

#### 原始 JSON 抽屉版式契约（索引 mapping / 模板模拟结果）

索引 tab 点索引名打开的「原始 mapping」抽屉，以及索引模板页的「模拟结果」抽屉，结构一样：一行说明 + 一个 `<pre class="es-source">`。这类抽屉同样要按「内容可以无限长、视口不会变长」设计。

改前面板吃的是全局的 `.es-source { max-height: 220px }`：抽屉 1294px 高时面板仍只有 220px，**下方空出 959px**；短 mapping（内容 243px）也因超过 220px 而带滚动条，长 mapping（内容 2456px）挤在 220px 小框里滚。用户口径是「只显示一部分、没有向下铺满」。

口径与上面的对话框一致：

> **抽屉 body = 不自滚的 flex 列 → 说明行不压缩（铁律）→ JSON 面板吃满剩余高度。**

`style.css` 1912-1914 的 `.es-raw-drawer` 三条规则之外，还有三处容易漏：

- **面板必须描边**。抽屉底色与 `.es-source` 的底色同为白，不描边的话「铺满了」在短 mapping 上根本看不出来，JSON 像是浮在白纸上（照抄 `.es-lc-detail .es-source` 与 `.es-qc-preview` 的边框 + 圆角）。
- **面板必须 `position: relative`**。`v-loading` 的遮罩是绝对定位的，面板自己不定位时遮罩会挂到最近定位祖先上——实测盖满整个抽屉（0 → 1294），连标题与说明行一起盖；补上定位后收敛到面板内（116 → 1273）。
- **类必须挂在承载面板的那个 `el-drawer` 上**。CSS 全靠 `.es-raw-drawer` 生效，漏挂或挂错位置都是**静默不生效**（面板照样封顶 220px，不报错），所以接线本身也要上锁。

实测（1294px 视口）：面板 220px → **1157px**，铺满 YES，短 mapping 面板内不再滚动，长 mapping 改在 1157px 大框里滚。契约在 `script/_fe_smoke.js` 第 14 项逐条断言（14a 样式 / 14b 三个抽屉的接线与顺序 / 14c 反向覆盖：凡是用带 `v-loading` 的 `.es-source` 面板的文件，其抽屉必须挂该类），`script/measure_es_raw_drawer.html` 渲染台带 `?fixed=true` / `?long=true` / `?loading=true` / `?shot=true` 四个开关做反向验证。
