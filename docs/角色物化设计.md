...
> 关联文档：`docs/大数据底座高可用设计.md`（§3.2 角色规划、§4.1 占位符、§4.2 分配算法）
...
- 不引入「单机 → HA 停机转换」（沿用 `大数据底座高可用设计.md` §五 结论，本期不做）。
...
| 文件 | 改动 |
|------|------|
| `docs/大数据底座高可用设计.md` | §3.2 / §4.2 / §4.3 同步指向本设计（角色规划升级为 RolePlan） |
| `docs/角色物化设计.md` | 本文档随实施同步更新 |
...
1. **热区冲突**：`stack_engine.go` / `stack_instance.go` / `stack_verify.go` 是高频改动文件，开工前需确认无并行会话在改同一文件（同 `大数据底座高可用设计.md` §八-7 约定）。
