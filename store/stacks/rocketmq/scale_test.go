...
// 2→1：只剩 1 台 = 无副本 + NameServer 单点，必须拦截（文案含「至少保留 2 台」）。
func TestScaleGuardBlocksBelowTwo(t *testing.T) {
	err := New().ScaleGuard(scaleCtx("cluster", 2, 1))
	if err == nil {
		t.Fatal("2→1 应被拦截")
	}
	if !strings.Contains(err.Error(), "至少保留 2 台") {
		t.Fatalf("文案: %v", err)
	}
}

// 3→2：仍不低于 2 台（保留 1 个 Slave），放行。
func TestScaleGuardAllowsTwo(t *testing.T) {
	if err := New().ScaleGuard(scaleCtx("cluster", 3, 1)); err != nil {
		t.Fatalf("3→2 应放行: %v", err)
	}
}
